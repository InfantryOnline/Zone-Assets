...
namespace InfServer.Script.GameType_Multi
{
    public partial class Coop
    {
        ...
        public void spawnRandomWave(Team team, Team targetTeam, int count)
        {
            Helpers.ObjectState warpPoint = _baseScript.findFlagWarp(_botTeam, true);
            Helpers.ObjectState openPoint = new Helpers.ObjectState();
            ...
            int ripper_count = (int)(count * 0.25);

            count = count - ripper_count;

            for (int i = 0; i < count; i++)
            {

                string type = "MarineV2";
                ...
                if (!spawnBot(team, targetTeam, type, openPoint, null))
                    Log.write(TLog.Warning, "Unable to spawn bot");
            }

            for (int i = 0; i < ripper_count; i++)
            {

                string type = "RipperV2";
        ...
        public void spawnMarineWave(Team team, Team targetTeam, int count)
        {
            Helpers.ObjectState warpPoint = _baseScript.findFlagWarp(_botTeam, true);
            Helpers.ObjectState openPoint = new Helpers.ObjectState();
            Random rand = new Random();
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (!spawnBot(team, targetTeam, "MarineV2", openPoint, null))
        ...
        public void spawnRipperWave(Team team, Team targetTeam, int count)
        {
            Helpers.ObjectState warpPoint = _baseScript.findFlagWarp(_botTeam, true);
            Helpers.ObjectState openPoint = new Helpers.ObjectState();
            Random rand = new Random();
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (!spawnBot(team, targetTeam, "RipperV2", openPoint, null))
                ...
                case 9:
                    {
                        if (!_firstRushWave) // problems with spawns
                        {
                            ...
                            spawnRandomWave(_botTeam,_baseScript._coop._team, max);
                ...
                case 19:
                    {
                        if (!_firstBoss)
                        {
                            ...
                            int max = Convert.ToInt32(playercount * 2.25);
                            spawnRandomWave(_botTeam, _baseScript._coop._team, max);
