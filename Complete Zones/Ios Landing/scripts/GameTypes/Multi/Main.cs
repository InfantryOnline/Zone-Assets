...
{	// Script Class
	...
	{	///////////////////////////////////////////////////
		...
		public int _lastCapture = 0;		// Tick at which last flag was captured
		public int _lastCaptureCutoff = 0;	// How long after a flag capture do we award kills
		public bool _warnCapture = true;
		...
		public bool botDeath(Bot dead, Player killer, int weaponID)
		{
			...
			if (killer != null)
			{
				...
				// credit for kill streak even if kill stat uncredited
				UpdateKiller(killer);
		...
		public bool playerModcommand(Player player, Player recipient, string command, string payload)
		{
			...
			if (command.Equals("addbot"))
			{
				...
				_coop.spawnBot(_coop._team, _coop._botTeam, payload, player._state, player);
			}

			if (command.Equals("medic"))
			{
				_coop.spawnBot(_coop._team, _coop._team, "MedicV2", player._state, player);
			}
		...
		public void UpdateDeath(Player victim, Player killer)
		{
			...
			if (StatsCurrent(victim).lastKillerCount >= 10)
			{
				_arena.sendArenaMessage(string.Format("{0}", killer != null
					? killer._alias + " has ended " + victim._alias + "'s kill streak."
					: victim._alias + "'s kill streak has ended.")
				, 7);
			}
